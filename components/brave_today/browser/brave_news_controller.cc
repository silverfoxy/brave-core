...
#include "brave/common/pref_names.h"  // TODO(petemill): move to this component
...
void BraveNewsController::IsFeedUpdateAvailable(
  ...
  feed_controller_.DoesFeedVersionMatch(displayed_feed_hash,
                                        std::move(callback));
}
...
void BraveNewsController::ConditionallyStartOrStopTimer() {
  ...
  if (is_enabled) {
    ...
    if (!timer_feed_update_.IsRunning()) {
      timer_feed_update_.Start(FROM_HERE, base::TimeDelta::FromSeconds(30),
